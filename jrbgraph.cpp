...
void addEdge_int(Graph graph, int v1, int v2, int weight)
{
	...
	else // dinh v1 da co tren cay
	{
		...
		jrb_insert_int(tree, v2, new_jval_i(weight));
		// cout << "them dinh " << v2 << endl;
	}
	// cout << "them dinh " << v1 << "  " << v2 << endl;
...
int getEdgeWeight_int(Graph graph, int v1, int v2)
{
...
void dijkstra(Graph graph, char *start, char *end)
{
	...
	while (!Q.empty())
	{
		...
		Q.pop();
		if (visitedList.find(v) != visitedList.end())
			continue;
...
void PRIM(Graph graph, char *start)
{
	...
	{
		min = 99999;
		// can tim canh co trong so nho nhat ma 1 dinh thuoc C v� dinh thuoc U
